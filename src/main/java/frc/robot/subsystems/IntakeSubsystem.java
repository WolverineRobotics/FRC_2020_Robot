...
public class IntakeSubsystem extends SubsystemBase {
    ...
    private DoubleSolenoid piston; // piston that opens the front intake. Forward=Intake Openened and
                                   // Reverse=Intake Closed

    /**
     * Variables helping with evaluating ball states.
     */
    public List<Ball> mag;
    public List<Ball> unfinishedDesto; // contains a list of balls who's destination is not actually the final
                                       // destination, but take detours to get to their final destination.
    public List<Ball> ballsToRemove; // contains a list of balls to remove after the executeMotors() method is done.
    ...
    public void setSpeeds(double entrySpeed, double curveSpeed, double verticalLowerSpeed, double verticalUpperSpeed) {
        ...
        verticalLower.set(MathUtil
                .clamp(verticalLowerSpeed * RobotConst.IntakeConst.LOWER_VERTICAL_SPEED_REDUCTION_RATIO, -1, 1));
        verticalUpper.set(MathUtil
    ...
    /**
     * Returns the amount of balls in the magazine, evaluted by the size of the
     * magazine List<>
     * 
    ...
    public boolean[] getSensors() {
        boolean[] sensors = { isSensorOneActivated(), isSensorTwoActivated(), isSensorThreeActivated(),
                isSensorFourActivated(), isSensorFiveActivated() };
        return sensors;
    }
     ...
     * 
     * @param toOpen - toOpen = true if you want piston to go forward and open the
     *               intake entry = false if you want piston to retract and close
     *               the intake entry
     */
    ...
    /**
     * 
     * @return the next empty position on the intake. For example:
     * 
     *         5 | 4 | 3 | 1 2 | ____|
     * 
     *         Let's say that only number 5 has a ball. The next empty location will
     *         be 4.
     * 
    ...
    /**
     * Ball class has two main variables: destination & position. Simply has its own
     * getter & seter for each Also automatically sets its position to Position.ONE
    ...
    /**
     * Position enum Has ids and motor possessios. Each Ball Position has its own
     * motor possession.
     * 
     * Also has a static component which keeps track of all other enums so you can
     * back track, forward track, get the position enum based off id, and check if a
     * position is after another position.
     */
    public enum Position {
        ONE(1.0, Motor.ENTRY, Motor.CURVE), ONE_TWO(1.5, Motor.CURVE), TWO(2.0, Motor.CURVE, Motor.LOWER_VERTICAL),
        TWO_THREE(2.5, Motor.LOWER_VERTICAL), THREE(3.0, Motor.CURVE, Motor.LOWER_VERTICAL),
        THREE_FOUR(3.5, Motor.LOWER_VERTICAL), FOUR(4.0, Motor.LOWER_VERTICAL, Motor.UPPER_VERTICAL),
        FOUR_FIVE(4.5, Motor.LOWER_VERTICAL), FIVE(5.0, Motor.UPPER_VERTICAL), FIVE_SIX(5.5, Motor.UPPER_VERTICAL),;
        ...
        /**
         * Checks if a Position is after another Position. Use instructions:
         * Position.isAfter(). Note that this is a static method.
         * 
                    ...
                    if (!currentPossessions.contains(possession)) {
                        switch (possession) {
                            case ENTRY:
                                ...
                                break;
                            case CURVE:
                                setCurveSpeed(mag.size() == 1 ? direction * 0.8 : direction * 0.6);
                                break;
                            case LOWER_VERTICAL:
                                boolean[] sen = getSensors();
                                if ((sen[3 - 1] && sen[4 - 1]) && mag.size() == 3) {
                                    ...
                                    setVerticalLowerSpeed(mag.size() == 1 ? direction * 0.8 : direction * 0.55);
                                }
                                break;
                            case UPPER_VERTICAL:
